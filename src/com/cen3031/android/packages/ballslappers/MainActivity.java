...
public class MainActivity extends SimpleBaseGameActivity implements IOnSceneTouchListener, IUpdateHandler, IOnMenuItemClickListener {
	...
	public void onCreateResources() {
		...
		//easter egg?
        if (easterEgg.toLowerCase().equals("lifeisgood")){ 
        	NUM_LIVES = 100000;
        }
        if (easterEgg.toLowerCase().equals("wtf")){ 
        	PADDLE_WIDTH = PADDLE_WIDTH * 4;
        }
        if (easterEgg.toLowerCase().equals("nyan")){ 
        	HomeScreenActivity.mediaPlayer.stop();
        	HomeScreenActivity.mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.nyan);
        	HomeScreenActivity.mediaPlayer.setLooping(true);
        	HomeScreenActivity.mediaPlayer.start();
        }
        if (easterEgg.toLowerCase().equals("awesome") || easterEgg.toLowerCase().equals("rick")){ 
        	HomeScreenActivity.mediaPlayer.stop();
        	HomeScreenActivity.mediaPlayer = MediaPlayer.create(getApplicationContext(), R.raw.rick);
        	HomeScreenActivity.mediaPlayer.setLooping(true);
        	HomeScreenActivity.mediaPlayer.start();
        }
        else
